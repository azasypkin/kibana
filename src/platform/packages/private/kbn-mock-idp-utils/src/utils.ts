...
export async function createSAMLResponse(options: {
  ...
  const uiamSessionTokens = options.serverless
...
function createUiamSessionTokens({
  ...
  projectType,
  roles,
  // TODO: Given name, family name, and email attributes aren't required, but at this point mistakenly marked as
  // such by ES, we'll drop them in the near future.
  givenName = 'Alfa',
  familyName = 'Beta',
  email = 'alfa.beta@elastic.co',
}: {
  username: string;
